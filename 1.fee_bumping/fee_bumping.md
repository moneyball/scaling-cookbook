...
Even wallets and services that do not themselves support creating transactions
that signal opt-in RBF or replacement transactions should present a clear
and accurate experience to their users when dealing with RBF transactions:
...
- if an unconfirmed transaction is part of a package of unconfirmed
  transactions, the service should present ancestor and descendant feerate of
  the transaction alongside its feerate, to allow users to more accurately
  predict the package's chance of inclusion in future blocks.
