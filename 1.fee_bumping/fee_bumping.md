...
the set of transactions that maximize their miner fee. If all unconfirmed
...
If a miner considered transactions independently when constructing her block,
...
Until 2012, the Bitcoin Core mining component did not consider fee-rate when
ordering transactions for inclusion in a block. Until then, transactions had
been ordered by 'prority' (the sum of the (value X coin age) for each
transaction input, divided by the transaction size). Bitcoin Core [PR #1590][]
changed the mining code to sort transactions by fee rate. [Version 0.7.0][],
released in September 2012 was therefore the first Bitcoin Core release with
any fee-optimized behavior for miners.
...
and benchmarking, and concerns that it cause a DOS vector against miners.
...
25 transactions or 101vkB. This changes was included in [V0.13][].
